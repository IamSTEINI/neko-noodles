...
func _ready() -> void:
	...
	var shape = chosen_area.get_node("CollisionShape2D").shape as RectangleShape2D
	...
	var pos2 = Entry2.global_position

	if global_position.distance_to(pos1) < global_position.distance_to(pos2):
		$NavigationAgent2D.target_position = pos1
	else:
		$NavigationAgent2D.target_position = pos2
	...
	if not $NavigationAgent2D.is_target_reached():
		...
		if not reached_entry:
			...
			if chosen_table != null:
				chosen_table.tableClaimed = true
